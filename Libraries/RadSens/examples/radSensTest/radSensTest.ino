...
void loop(){
  ...
  radSens.get(); //The second way to interact with the sensor is to read all parameters into internal variables
  
  Serial.print("Rad intensy dyanmic: ");

  Serial.println(radSens.IntensyDyanmic); /*Saved value of radiation intensity (dynamic period T < 123 sec).*/

  Serial.print("Rad intensy static: ");
  
  Serial.println(radSens.IntensyStatic); /*Saved value of radiation intensity (static period T = 500 sec).*/

  Serial.print("Number of pulses: ");

  Serial.println(radSens.pulses); /*Saved value of accumulated number of pulses registered by the 
                                                 module since the last I2C data reading.*/
  delay(2000);
...
/*
Note on using internal variables
Calling the get () function updates the variables intensystatic, intensydyanmic, hvgenstate, and pulses
Calling the get (1) function updates all internal variables:
IntensyStatic, IntensyDyanmic, HVGenState, pulses, chip_id, firmware_ver, I2C_addr/
*/
