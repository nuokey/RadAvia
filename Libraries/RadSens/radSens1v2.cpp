...
{
    Wire.requestFrom(_sensor_address, (char)19);
    for (int i = 0; i < 19; i++)
    {
        _data[i] = Wire.read();
    }
...
bool ClimateGuard_RadSens1v2::get(){
    if (updateData())
    {
        IntensyStatic = ((_data[6] << 16) | (_data[7] << 8) | _data[8]) / 10.0;
        IntensyDyanmic = ((_data[3] << 16) | (_data[4] << 8) | _data[5]) / 10.0;
        HVGenState =  _data[17];
        return 1;
    }
    return 0;
}
bool ClimateGuard_RadSens1v2::get(bool full){
    if (updateData())
    {
        if(full){
            chip_id = _data[0];
